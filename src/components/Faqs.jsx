...
export function Faqs() {
  return (
    <section
      ...
      className="relative overflow-hidden bg-slate-50 py-20 sm:py-32 dark:bg-slate-900 dark:text-white/80"
    >
      ...
      <Container className="relative">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2
            id="faq-title"
            className="font-display text-3xl tracking-tight dark:text-white sm:text-4xl"
          >
            Frequently asked questions
          </h2>
          <p className="mt-4 text-lg tracking-tight dark:text-white/90">
            ...
            <li key={columnIndex}>
              <ul role="list" className="flex flex-col gap-y-8">
                {column.map((faq, faqIndex) => (
                  <li key={faqIndex}>
                    <h3 className="font-display text-lg leading-7 dark:text-white">
                      {faq.question}
                    </h3>
                    <p className="mt-4 text-sm dark:text-white/80">{faq.answer}</p>
                  </li>
                ))}
              </ul>
            </li>
